...
export class GoodsDetailPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public httpService: HttpServicesProvider, public storage: StorageProvider,public config:ConfigProvider) {

    console.log('商品ID: '+this.navParams.data.id);
    this.getGoodsDetail(this.navParams.data.id);
  ...
  addCart() {
    ...
    var storageData = this.storage.getItem('carts_data');
    ...
    if (storageData) {  /*有数据*/
      ...
      //重新写入
      this.storage.setItem('carts_data', storageData);
      ...
      tempArr.push(json);
      this.storage.setItem('carts_data', tempArr);
