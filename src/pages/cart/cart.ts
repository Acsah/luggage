...
export class CartPage {
  ...
  getCartsData() {
    var cartsData = this.storage.getItem('carts_data');
  ...
  //离开的时候保存购物车数据
  ionViewWillLeave() {
    this.storage.setItem('carts_data', this.list);
  }
  ...
  doDelete() {
    ...
    //重新写入localstorage
    this.storage.setItem('carts_data', noCheckedArr);
  ...
  doPay() {
    ...
    if (tempArr.length > 0) {
      this.storage.setItem('order_data', tempArr);
